...
float evaluate(string tokens) {
    ...
    for (i = 0; i < tokens.length(); i++) {
        if (tokens[i] == '(') { // 여는 괄호가 있으면 스택에 추가합니다.
            ops.push(tokens[i]);
        } else if (isdigit(tokens[i])) { // 현재 문자가 numeric value인지
                                         // 검사합니다.
            float val = 0;
            ...
            while (i < tokens.length() &&
                   (isdigit(tokens[i]) ||
                    (tokens[i] == '.'))) // 실수값에 포함되지 않을 문자(괄호나
                                         // 연산자)가 나올 때까지 이동합니다.
                i++;
            val_str = tokens.substr(
                start, i - start); // i 직전 인덱스 까지가 float으로 변환될
                                   // 문자열 입니다.
            val = stof(val_str); // 문자열을 float으로 변환합니다.
            values.push(val);    // 실수 값 스택에 추가합니다.
            i--; // 현재 numeric value 이후 문자를 가리키고 있으므로 인덱스를
                 // 하나 감소시킵니다.
        } else if (tokens[i] == ')') // 닫는 괄호가 나오면
        {
            while (!ops.empty() &&
                   ops.top() !=
                       '(') // 연산자가 비어있지 않고, 스택의 최상위 원소가 여는
                            // 괄호가 아니면 다음 반복문을 수행합니다.
            {
                // 스택에서 두 수를 꺼내고 연산자를 꺼내 연산 값을 값 스택에
                // 다시 넣습니다.
...
bool balanceTest(string raw) {
    ...
    for (int i = 0; i < raw.length(); i++) {
        ...
        if (raw[i] == '(' || raw[i] == '[' || raw[i] == '{') {
            ...
            // 스택 최상위 문자가 각 닫는 괄호에 상응하는 여는 괄호가 아니면
            // false를 반환합니다.
            switch (raw[i]) {
            case ')':
                x = s.top();
    ...
    // 모든 문자열 검사가 끝난 후 스택이 비어있으면 (모든 괄호가 쌍을 알맞게
    // 찾고 마무리 되면) empty 입니다.
    return (s.empty());
}
